...
async function main() {
  const variants = await prisma.variant.findMany({
    where: { sku: { contains: 'GREIGE' } },
    select: { sku: true, imageUrl: true },
  });
  console.log('Found:', JSON.stringify(variants, null, 2));

  const target = variants.find(v => v.sku.includes('FRE') || v.sku.includes('FRESCO'));
  if (!target) { console.log('No Fresco variant found'); await prisma.$disconnect(); await pool.end(); return; }

  await prisma.variant.update({
    where: { sku: target.sku },
    data: { imageUrl: 'https://arriscraft.com/wp-content/uploads/sites/3/2020/10/Fresco_Greige-e1655756149274.jpeg' },
  });
