...
async function main() {
  const variants = await prisma.variant.findMany({
    where: { sku: { contains: 'CARBON' } },
    select: { sku: true, imageUrl: true },
  });
  console.log('Found:', JSON.stringify(variants, null, 2));

  const target = variants.find(v => v.sku.includes('CRN') || v.sku.includes('RNS') || v.sku.includes('REN'));
  ...
  await prisma.variant.update({
    ...
    data: { imageUrl: 'https://arriscraft.com/wp-content/uploads/sites/3/2022/06/Renaissance_22_Carbon.jpeg' },
  });
