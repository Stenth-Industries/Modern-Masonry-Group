...
export default function Catalogue() {
  ...
  const fetchProducts = useCallback(async () => {
    ...
    try {
      ...
      if (data.success) {
        setProducts((prev) =>
          queryParams.page === 1 ? data.data : [...prev, ...data.data],
        );
        setMeta(data.meta);
      }
    } catch (err) {
    ...
    <div className="container mx-auto px-4 py-8 max-w-7xl">
      <header className="mb-10 text-center md:text-left">
        ...
        <p className="text-[var(--text-secondary)] text-lg max-w-2xl">
          Explore our premium collection of bricks and masonry materials.
        ...
        <main className="lg:w-3/4">
          {products.length > 0 ? (
            <>
                      ...
                      <Card className="flex flex-col h-full group">
                        <div className="relative h-48 bg-[var(--bg-primary)] border-b border-[var(--text-secondary)]/20 overflow-hidden flex items-center justify-center">
                          ...
                          {product.variants?.[0]?.stock && (
                            <div className="absolute top-3 left-3 z-10 bg-[var(--bg-secondary)]/90 backdrop-blur text-xs px-2 py-1 rounded font-medium text-[var(--text-primary)] border border-[var(--text-secondary)]/30 shadow-sm flex items-center gap-1.5">
                              <span
                                className={`w-1.5 h-1.5 rounded-full ${product.variants[0].stock.toLowerCase() === "in stock" ? "bg-green-500" : "bg-amber-500"}`}
                              ></span>
                              {product.variants[0].stock}
                            </div>
                          )}
                          ...
                          {product.variants?.[0]?.imageUrl ? (
                            ...
                            <div className="w-full h-full bg-gradient-to-br from-[var(--bg-secondary)] to-[var(--bg-primary)] flex items-center justify-center">
                              <span className="text-[var(--text-secondary)] opacity-50 font-medium">
                                No Image
                              </span>
                            </div>
                          )}
                        ...
                        <div className="p-5 flex flex-col flex-grow">
                          <div className="mb-2 flex flex-wrap items-center gap-x-2">
                            {product.manufacturers?.map((m) => (
                              <span
                                key={m.id}
                                className="text-xs uppercase tracking-wider text-[var(--accent)] font-semibold"
                              >
                                {m.name}
                              </span>
                            ))}
                          ...
                          <div className="mt-auto pt-4 flex flex-wrap gap-2">
                            {product.categories
                              ?.filter((c) => c.type === "colour")
                              .map((c) => (
                                <div
                                  key={c.id}
                                  className="flex items-center gap-1.5 text-xs bg-[var(--bg-primary)] px-2 py-1 rounded-md border border-[var(--text-secondary)]/10"
                                >
                                  {c.hexCode && (
                                    <span
              ...
              {loading && queryParams.page > 1 && (
                <div className="py-8 flex justify-center items-center">
                  ...
                  <span className="ml-3 text-[var(--text-secondary)] font-medium">
                    Loading more styles...
                  </span>
                </div>
              )}
            ...
            <div className="flex flex-col items-center justify-center p-12 text-center h-64 border border-dashed border-[var(--text-secondary)]/30 rounded-xl">
              <svg
                className="w-16 h-16 text-[var(--text-secondary)]/50 mb-4"
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
              >
