...
export default function BrickCatalogue({ navigate, initialQuery = "", initialPage = 1 }) {
  ...
  // Quick-jump dropdown: separate debounced fetch that doesn't affect the main grid
  useEffect(() => {
    clearTimeout(searchDebounceRef.current);
    if (query.trim().length < 2) { setSearchResults([]); setSearchDropdownOpen(false); return; }
    searchDebounceRef.current = setTimeout(async () => {
      try {
        const res = await fetch(`/api/products?search=${encodeURIComponent(query.trim())}&material=Brick&limit=5`);
      ...
      <div className="relative z-10 flex flex-col flex-grow">
        ...
        <div className="w-full bg-black/80 backdrop-blur-xl border-y border-[rgba(255,255,255,0.06)] px-4 sm:px-8 xl:px-14 py-5 grid grid-cols-[auto_1fr_auto] items-center gap-2 sm:gap-4 z-40 sticky top-0 shadow-[0_15px_40px_rgba(0,0,0,0.5)]">
          ...
          <div className="hidden md:flex items-center justify-center gap-2 overflow-x-auto scrollbar-none">
            ...
            {[...types, ...colors, ...finishes, ...series, ...manufacturers].length > 1 && (
              <button
                    ...
                    {searchResults.map((product) => {
                      ...
                      return (
                        <button
                          key={product.id}
                          type="button"
                          onClick={() => { navigate('#brick-detail/' + product.id); setSearchDropdownOpen(false); }}
          ...
          >
            ...
            {!loading && !showFavourites && !errorMsg && products.length === 0 && (
              ...
              >
                ...
                <div className="w-12 h-px bg-[#c9a449]/30" />
                <button
                  onClick={() => { setTypes([]); setColors([]); setFinishes([]); setSeries([]); setManufacturers([]); setQuery(""); }}
